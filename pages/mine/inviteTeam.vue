	...
	<view class="invite_page">
		...
		<uni-popup :show="type === 'bottom_share'" position="middle" :shareStatus="true" @hidePopup="hidePopup">
			<view class="shareGroup">
				<!-- 遮罩 -->
				<image :src="baseUrl + '/api/activity/getCode?url=http://haoyufu.cn/afx/download?inviteCode=' + inviteCode" mode="widthFix" @tap="hidePopup"></image>
			...
			switch (Number(e.currentTarget.id)) {
				...
				case 0:
					uni.share({
						...
						type: 2,
						imageUrl: self.baseUrl + '/api/activity/getCode?url=http://haoyufu.cn/afx/download?inviteCode=' + self.inviteCode,
						success: function(res) {
							console.log('success:' + JSON.stringify(res));
						},
						fail: function(err) {
				...
				case 3:
					uni.setClipboardData({
						data: 'http://haoyufu.cn/afx/download?inviteCode=' + self.inviteCode,
						complete() {
							uni.showToast({
								title: '已复制到剪贴板'
							});
